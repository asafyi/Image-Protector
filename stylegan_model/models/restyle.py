...
from models.encoders import fpn_encoders, restyle_psp_encoders, restyle_e4e_encoders
...
class REStyle(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.load_weights()
        self.avg_image_ = None

    def get_avg_image(self):
        if self.avg_image_ is None:
            avg_image = self(self.latent_avg.unsqueeze(0),
                            input_code=True,
                            randomize_noise=False,
                            return_latents=False,
                            average_code=True)[0]
            self.avg_image_ = avg_image.to(self.opts.device).float().detach().unsqueeze(0)
        return self.avg_image_
