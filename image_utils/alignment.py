...
def embed_faces(img, imgs_list, points_list):
    ...
    for face, points in zip(imgs_list, points_list):
        ...
        pts_face = np.array([[0, 0], [0, face.shape[0]], [face.shape[1], face.shape[0]], [face.shape[1], 0]], dtype=np.float32)
        ...
        warped = cv2.warpPerspective(face, M, (img.shape[1], img.shape[0]), flags=cv2.INTER_CUBIC)   
